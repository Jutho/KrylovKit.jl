...
function LinearAlgebra.mul!(w::InnerProductVec{F}, a::Number, v::InnerProductVec{F}) where F
    mul!(w.vec, a, v.vec)
    return w
end

function LinearAlgebra.mul!(w::InnerProductVec{F}, v::InnerProductVec{F}, a::Number) where F
    mul!(w.vec, v.vec, a)
    return w
end
...
function LinearAlgebra.axpy!(a::Number, v::InnerProductVec{F}, w::InnerProductVec{F}) where F
    axpy!(a, v.vec, w.vec)
    return w
end
function LinearAlgebra.axpby!(a::Number, v::InnerProductVec{F}, b, w::InnerProductVec{F}) where F
