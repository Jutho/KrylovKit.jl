...
function geneigsolve(AB::Tuple{AbstractMatrix,AbstractMatrix}, howmany::Int = 1,
        which::Selector = :LM, T = promote_type(eltype.(AB)...); kwargs...)
    ...
    geneigsolve(AB, rand(T, size(AB[1],1)), howmany::Int, which; kwargs...)
end
geneigsolve(AB::Tuple{Any,AbstractMatrix}, howmany::Int = 1, which::Selector = :LM,
            T = eltype(AB[2]); kwargs...) =
    geneigsolve(AB, rand(T, size(AB[2],1)), howmany, which; kwargs...)
geneigsolve(AB::Tuple{AbstractMatrix,Any}, howmany::Int = 1, which::Selector = :LM,
...
function geneigsolve(f, x₀, howmany::Int = 1, which::Selector = :LM; kwargs...)
    ...
    geneigsolve(f, x₀, howmany, which, alg)
end
...
function geneigselector(f, T::Type; issymmetric = false, ishermitian = issymmetric,
