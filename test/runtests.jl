...
module MinimalVecs
    ...
    wrapop(A::AbstractMatrix) =
        (v, flag = Val(false))->wrapvec(flag === Val(true) ? A'*unwrapvec(v) : A*unwrapvec(v))
...
#     wrapvec(v) = v
#     unwrapvec(v) = v
#     wrapop(A::AbstractMatrix) = (v->A*v, v->A'*v)
#
#     t = time()
#     include("svdsolve.jl")
#     t = time() - t
#     println("svdsolve with two functions argument: tests finisthed in $t seconds")
# end
