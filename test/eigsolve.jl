...
@testset "Arnoldi - eigsolve full" begin
    ...
    @testset "MinimalVec{$IP}" for IP in (true, false)
        ...
        n1 = div(n, 2)
        D1, V1, info = eigsolve(wrapop(A), v, n1, :SI, alg)
        n2 = n - n1
        D2, V2, info = eigsolve(wrapop(A), v, n2, :LI, alg)
        D = sort(eigvals(A); by=imag)
...
@testset "Arnoldi - eigsolve iteratively" begin
    ...
    @testset "MinimalVec{$IP}" for IP in (true, false)
        ...
        D1, V1, info1 = eigsolve(wrapop(A), v, n, :SI, alg)
        D2, V2, info2 = eigsolve(wrapop(A), v, n, :LI, alg)
        D = eigvals(A)
